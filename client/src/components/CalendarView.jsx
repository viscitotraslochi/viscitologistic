...
import {
  Paper, Typography, Box, Button, useMediaQuery, useTheme, Tooltip, Chip, Divider,
    ...
    try {
      ...
      const formattedEvents = response.data.map(job => {
        const formatToISODate = (dateInput) => {
          ...
          // sv-SE restituisce YYYY-MM-DD (evita vari problemi di parsing)
        ...
        return {
          ...
          extendedProps: {
            ...job,
            date: dateStr,      // fondamentale per il modale
            end_date: endDateStr
          },
          backgroundColor: job.deposit
            ? '#2e7d32'
            : (String(job.job_type || job.tipo_lavoro || job.tipo || '').toLowerCase() === 'sopralluogo'
              ? '#ffa000'
              : '#1976d2')
        };
      });
        ...
        >
          ...
          <Box sx={{
            ...
            <Box sx={{ display: 'flex', flexDirection: isMobile ? 'row' : 'column', alignItems: 'center', gap: 0.5, opacity: 0.8 }}>
              {isMobile ? <KeyboardArrowRightIcon sx={{ color: mainColor }} /> : <KeyboardArrowDownIcon sx={{ color: mainColor }} />}
              <Chip
                label={durationLabel}
                size="small"
                sx={{
